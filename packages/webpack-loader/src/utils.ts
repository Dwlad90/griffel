 ...
 * @returns
 */
export const setPluginConfiguredOption = (rules: (RuleSetRule | '...')[], pluginName: string): void => {
  for (const rule of rules) {
    ...
    if (Array.isArray(use)) {
      ...
      for (const nestedUse of use) {
        if (typeof nestedUse !== 'object' || nestedUse.loader !== '@griffel/webpack-loader') {
          continue;
        }
      ...
      if (typeof use === 'object' && use.loader === '@griffel/webpack-loader') {
        const { options } = use;
